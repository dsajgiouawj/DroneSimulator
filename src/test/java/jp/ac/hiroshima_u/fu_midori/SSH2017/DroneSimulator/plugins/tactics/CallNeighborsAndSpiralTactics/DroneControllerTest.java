...
public class DroneControllerTest {
    ...
    public static class spiral2の時 {
        ...
        public void executeTurnを実行すると螺線探索() throws Exception {
            ...
            Point2D actual = sut.getPoint();
            DroneImpl droneForSpiral = new DroneImpl(VIEW_RANGE_RADIUS, new ArrayList<>());
            while (!droneForSpiral.canMove()) {
                droneForSpiral.nextTurn();
                droneForSpiral.goToPoint(spiral2Center);
            }
            droneForSpiral.nextTurn();
            SpiralTacticsDrone spiralTacticsDrone = new SpiralTacticsDrone(droneForSpiral, spiral2NumDrone, spiral2ID, VIEW_RANGE_RADIUS, SEARCH_RATIO2, spiral2Center);
            spiralTacticsDrone.executeTurn();
            Point2D expected = droneForSpiral.getPoint();
            assertThat(actual, is(expected));
        }
